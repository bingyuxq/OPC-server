...
using static OPC_server.Program;
...
namespace OPC_server
{
    ...
    public class OpcUaNodeManager : CustomNodeManager2
    {
        ...
        public OpcUaNodeManager(IServerInternal server, ApplicationConfiguration configuration)
            : base(server, configuration, Program.namespaceUris)
        {
        ...
        public override void CreateAddressSpace(IDictionary<NodeId, IList<IReference>> externalReferences)
        {
            lock (Lock)
            {
                ...
                m_refreshTimer = new Timer(refreshData, null, 10000, 10000);
            }
        }
        ...
        private ServiceResult OnWriteInterval(ISystemContext context, NodeState node, ref object value)
        {
            try
            {
                ...
                {
                    m_refreshTimer.Change(100, (int)m_simulationInterval);
                }
            ...
            {
                Console.WriteLine(e.Message, "Error writing Interval variable.");
        ...
        private ServiceResult OnWriteEnabled(ISystemContext context, NodeState node, ref object value)
        {
            ...
            {
                Console.WriteLine(e.Message, "Error writing Enabled variable.");
        ...
        private void load(IList<IReference> references)
        {
            ...
            foreach (RedisValue prefix in opcUaNamespace)
            {
                ...
                FolderState root = CreateFolder(null, prefixStr, prefixStr);
                root.AddReference(ReferenceTypes.Organizes, true, ObjectIds.ObjectsFolder);
                references.Add(new NodeStateReference(ReferenceTypes.Organizes, false, root.NodeId));
                root.EventNotifier = EventNotifiers.SubscribeToEvents;
                AddRootNotifier(root);

                IEnumerable<RedisKey> tags = RedisConnection.server.Keys(pattern: prefix.ToString() + "*");
                foreach (RedisKey key in tags)
                {
            ...
            //Console.WriteLine(opcUaNamespace[0].ToString());
